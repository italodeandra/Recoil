...
var Recoil_RecoilValueClasses = {
    ...
    RecoilState: class extends AbstractRecoilValue {},
    RecoilValueReadOnly: class extends AbstractRecoilValue {},
  },
...
var Recoil_Node = {
  ...
  registerNode: function (node) {
    ...
    return (
      ...
      null == node.set
        ? new require$$1.RecoilValueReadOnly(node.key)
        : new require$$1.RecoilState(node.key)
    );
  },
...
var Recoil_RecoilValue = {
  ...
  AbstractRecoilValue: AbstractRecoilValue$1,
  RecoilState: RecoilState$1,
  peekRecoilValueAsLoadable: function (store, {key: key}) {
    const state = store.getState().currentTree;
  ...
  setRecoilValue: function (store, {key: key}, newValue) {
...
var Recoil_isNode = function (object) {
  ...
  const defaultView =
    null !==
      (_doc$defaultView = (null != object
        ? null !== (_ownerDocument = object.ownerDocument) &&
          void 0 !== _ownerDocument
          ? _ownerDocument
          : object
        : document
      ).defaultView) && void 0 !== _doc$defaultView
      ? _doc$defaultView
      : window;
  return !(
    null == object ||
...
var Recoil_deepFreezeValue = function deepFreezeValue(value) {
  if (
    ...
    !(function (value) {
      ...
      return (
        null != value['@@__IMMUTABLE_ITERABLE__@@'] ||
        null != value['@@__IMMUTABLE_KEYED__@@'] ||
        null != value['@@__IMMUTABLE_INDEXED__@@'] ||
        null != value['@@__IMMUTABLE_ORDERED__@@'] ||
        null != value['@@__IMMUTABLE_RECORD__@@'] ||
        !!Recoil_isNode(value) ||
        !!Recoil_isPromise(value)
      );
    })(value)
  ) {
    Object.freeze(value);
      ...
      for (const upstreamNode of addedDeps)
        newState = {
          ...
          nodeToNodeSubscriptions: mapByUpdatingInMap$2(
            ...
            upstreamNode,
            subs => setByAddingToSet$2(null != subs ? subs : emptySet$2, key),
          ),
        };
    ...
    return (
      (function (store, cacheKey, loadable) {
        'loading' !== loadable.state
          ? !0 == !options.dangerouslyAllowMutability &&
...
const {loadableWithValue: loadableWithValue$2} = Recoil_Loadable,
  ...
  {isRecoilValue: isRecoilValue$2} = Recoil_RecoilValue,
  {
              ...
              : {
                  ...
                  validator: storedValue =>
                    storedValue instanceof DefaultValue$1
                      ...
                      : Recoil_nullthrows(
                          options.persistence_UNSTABLE,
                        ).validator(storedValue, DEFAULT_VALUE$2),
                },
        });
    ...
    useEffect(() => {
      Recoil_Queue.enqueueExecution('Batcher', () => {
        const storeState = storeRef.current.getState(),
          {currentTree: currentTree, nextTree: nextTree} = storeState;
        null !== nextTree &&
          (nextTree.dirtyAtoms.size &&
...
var Recoil_intersectSets = function (first, ...rest) {
  const ret = new Set();
...
const {
  ...
  {setByAddingToSet: setByAddingToSet$4} = Recoil_CopyOnWrite,
  {
    ...
    useMemo(() => {
      function useSetRecoilState(recoilState) {
        return newValueOrUpdater => {
          ...
          const storeState = storeRef.current.getState(),
            newValue = valueFromValueOrUpdater(
              storeRef.current,
      ...
      function useRecoilValueLoadable(recoilValue) {
        return (
          ...
          getRecoilValueAsLoadable$1(storeRef.current, recoilValue)
        );
      }
...
var Recoil_Hooks = {
  ...
  useTransactionSubscription: useTransactionSubscription,
  useSnapshotWithStateChange: function (transaction) {
    const storeRef = useStoreRef$1();
    let snapshot = (function () {
      const [_, setState] = useState$1(0);
      return (
        useTransactionSubscription(useCallback(() => setState(x => x + 1), [])),
        cloneState(useStoreRef$1().current.getState().currentTree, {
          isSnapshot: !0,
        })
      );
    })();
    transaction(({key: key}, updater) => {
    ...
    const atomValues = Recoil_mapMap(snapshot.atomValues, v => v.contents),
      updatedAtoms = Recoil_intersectSets(
        snapshot.dirtyAtoms,
        new Set(atomValues.keys()),
      );
    return {atomValues: atomValues, updatedAtoms: updatedAtoms};
  },
  useTransactionObservation: function (callback) {
    useTransactionSubscription(
      useCallback(
        (store, previousState) => {
          let nextTree = store.getState().nextTree;
          nextTree || (nextTree = store.getState().currentTree);
          const atomValues = externallyVisibleAtomValuesInState(nextTree),
            ...
            modifiedAtoms = new Set(nextTree.dirtyAtoms);
          callback({
